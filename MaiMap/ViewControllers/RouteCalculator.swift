...
class RouteCalculator {
    ...
    private var graph: [String: [String: CGFloat]] = [:]
    ...
    private func buildGraph() {
        ...
        for point in points {
            ...
            for connId in point.connections {
                if let neighbor = points.first(where: { $0.id == connId }), neighbor.floor == point.floor {
                    let distance = calculateDistance(point, neighbor)
                    graph[point.id]?[connId] = distance
                    graph[connId]?[point.id] = distance // Двусторонняя связь
                } else {
                    print("Warning: Invalid horizontal connection from \(point.id) to \(connId)")
                }
            }
        }

        // Вертикальные соединения (лифты, лестницы)
    ...
    func findPath(from startId: String, to endId: String) -> [String]? {
        ...
        var iteration = 0
        let maxIterations = 1000 // Ограничение итераций
        ...
        while !openSet.isEmpty && iteration < maxIterations {
            ...
            for (neighborId, weight) in graph[currentId] ?? [:] {
                ...
                if tentativeGScore < gScore[neighborId, default: .infinity] {
                    ...
                    fScore[neighborId] = tentativeGScore + heuristic(points.first(where: { $0.id == neighborId })!, end)
                    openSet.insert(neighborId)
                }
            }
            iteration += 1
        }
        print("Путь не найден или превышен лимит итераций")
        return nil
    }
    ...
    private func heuristic(_ a: Point, _ b: Point) -> CGFloat {
        let dx = a.x - b.x
        let dy = a.y - b.y
        let dz = CGFloat(a.floor - b.floor) * 1000 // Штраф за этажи
